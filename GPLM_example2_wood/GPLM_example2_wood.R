...
#Wood (2013)  f0 
#wood f0 n=100 set.seed(201501011)
set.seed(201501011)
#Wood f0 n=200 set.seed(20140811)

#wood f1
#wood f1 n=100 set.seed(5080888)
#wood f1 n=200 set.seed(20140808)

#wood f2
#wood f2 n=100  set.seed(25080888)
#wood f2 n=200  set.seed(201206195)
...
tgrid <- seq(0.0, 1.0, 0.005) #length 201
...
for (j in 1:Nsim) {
  ...
  #wood f2
  #eta <-  (2*(10^5)*(xt^11)*((1-xt)^6) + (10^4)*(xt^3)*((1-xt)^10))
  ...
  for (hi in 1: hlength) {
    ...
    AICc[j, hi] <- log(intDall[j,hi]/ samplen) + 2*(degfee[j, hi] +1) /(samplen - degfee[j, hi] -2)
    if  (AICc[j, hi] < AICcmin) { AICcmin <- AICc[j, hi] 
    AICch[j] <- h
    }
    ...
    if  (pvalue[j,hi] < adaptivepvalue[j]) { adaptivepvalue[j] <- pvalue[j,hi] 
    adaptiveh[j]<- h
    }
...
degresults<-  round(matrix(c(mean(degfee[,1]), sd(degfee[,1]), mean(degfee[,2]), sd(degfee[,2]),
                             ...
                             mean(degfee[,5]), sd(degfee[,5]), mean(degfee[,6]), sd(degfee[,6]),
                             mean(degfee[,7]), sd(degfee[,7])), nrow=2,byrow=FALSE),6);degresults
