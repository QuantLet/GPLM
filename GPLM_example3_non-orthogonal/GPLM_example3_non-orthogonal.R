...
#smoother matrix in Huang and Chen (2008)
# Epanechnikov kernel	
#normlizing & symmatric & sum to one
proj.ep.symmetric<- 
  function(x, xgrid, h)
  {
    gridlength <- length(xgrid)
...
#using Epanechnikov kernel, xgrid must be equally spaced
# testing no effect of xt
#x1 x2 linear part 
#xt nonparametric part
anodev.gplm.logit.chisq <- function(y, x1, x2, xt, tgrid, h)
{
  ...
  while ((bdiff >0.001) & (nitrt <30)) {
    ...
    for (j in 1:length(tgrid)) {
      ...
      # keep parametric part fixed, and fit local linear for xt
      #set maxit=5, it is enough to update the values
      #offset(I(1.3 * X1))
      d4.glm<- glm(factor(y)~ offset(bcoeff[1]*x1)+offset(bcoeff[2]*x2)+xt.local , family = binomial(link = "logit"), weights=kweights, control=glm.control(maxit=5))
    ...
    # keep nonparametric part fixed, and fit linear terms for x1 and x2
    #set maxit=5, it is enough to update the values
    para.glm<- glm(factor(y)~ -1+ x1+x2+offset(thetastar) , family = binomial(link = "logit"), control=glm.control(maxit=5))
  ...
  degfree <- d4.H$defree 
  chipvalue<- 1.0- pchisq(teststat0, degfree-1)
  
  list(intDev=intDev, teststat0 = teststat0, bcoeff=bcoeff,  thetastar=thetastar, degfree=degfree, chipvalue=chipvalue, notconv=notconv)
...
#partial linear exp example 3
#exp nonlinear trend
#a=3 n=200  
set.seed(20121025)

#a=2 n=200  set.seed(20130224)
#a=1.0  n=200 set.seed(20130301)
#a=0.0 n=200 set.seed(20130304)

#a=0.0 n=100  set.seed(201501233)
#a=1 n=100 set.seed(201502233)
#a=2 n=100   set.seed(201502566)
#a=3 n=100 set.seed(201502588)
...
tgrid <- seq(-0.5, 1.0, 0.005) #length 301
...
for (k in 1:Nsim) {
  ...
  varmat<- matrix(c(1.0, 0.3,0.3, 1.0),2,2)
  
  ########## random design check if every neighborhood has at least 5 points at smallest h #############
  ...
  for (hi in 1: hlength) {
    h<- hchoice[hi]
    #call the function
    semilogit<- anodev.gplm.logit.chisq(d4$y0 , d4$x1.th, d4$x2.th, d4$xt, tgrid, h)
    ...
    # if non-convergent
    if (semilogit$notconv==1)  conv[k, hi] <-0
    ...
    AICc[k, hi] <- log(intD[k,hi]/ samplen) + 2*(degfee[k, hi] +1) /(samplen - degfee[k, hi] -2)
    if  ((AICc[k, hi] < AICcmin) && (conv[k,hi]==1)) { AICcmin <- AICc[k, hi] 
    AICch[k] <- h
    }
    
    #find the smallest pvalue based on Horowitz and Spokoiny (2001)
    if  ((pvalue[k,hi] < adaptivepvalue[k]) && (conv[k,hi]==1)) { adaptivepvalue[k] <- pvalue[k,hi] 
    adaptiveh[k]<- h
    }
    
  } 
  #end hi bandiwdth
  #use convergent h to select AICch
  if (any(conv[k,]==1)) {
    findaicc<- (hchoice==AICch[k] )
    AICcpvalue[k] <- pvalue[k, findaicc]
  }
  else {cat("all h values fail to converge\n")}
...
testresults1<- round(matrix(c(hchoice, colSums(pvalue<=0.05)/Nsim),nrow=2,ncol=hlength, byrow=TRUE),6); testresults1
...
degresults<-  round(matrix(c(mean(degfee[,1]), sd(degfee[,1]), mean(degfee[,2]), sd(degfee[,2]),
                             ...
                             mean(degfee[,5]), sd(degfee[,5])), nrow=2,byrow=FALSE),6);degresults
...
testresults3<- round(matrix(c(hchoice, ((colSums((pvalue*conv>0) & (pvalue*conv <=0.05)))/colSums(conv))),nrow=2,ncol=hlength, byrow=TRUE),6)
print(testresults3)
...
print(round(matrix(c(hchoice, testresults4),nrow=2,ncol=hlength, byrow=TRUE),6))
